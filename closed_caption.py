...
class StatementBody(object):
  ...
  def parse_contents(f, bytes_to_read):
    """
    if len(f) > 3:
      print("Byte array of characters:")
      b_array = f[3:]
      print(b_array)
      for i in range (0, len(b_array), 2):
        ...
        char2 = b_array[i+1]
        try:
          print(">", bytes((char1, char2)).decode('euc-jp'))
          ...
          if gaiji == 0x7c21:
            ...
            continue
          try:
            print(">>", GAIJI_MAP[gaiji])
          except KeyError:
            print(">>>", '(0x{:x}{:x})'.format(char1, char2))
    """
    '''
...
class DataUnit(object):
  ...
  def size(self):
    ...
    '''
    return self._data_unit_size[0] + 5
